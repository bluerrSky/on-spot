#Termux port to make it work as a daemon in phone
...
top_artists = [artist for artist,duration in sorted_duration[:9]]
new_data = {}
...
data = new_data
dates = [datetime.strptime(list(track.values())[0], '%Y-%m-%d %H:%M:%S') for track in date_with_tracks]
...
def update_pie(something):
    ...
    i += 1
ani=  animation.FuncAnimation(fig,update_pie,frames=100,repeat=True)
plt.show()